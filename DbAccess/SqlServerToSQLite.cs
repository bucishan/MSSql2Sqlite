...
namespace DbAccess
{
    ...
    public class SqlServerToSQLite
    {
        ...
        public static void ConvertSqlServerToSQLiteDatabase(string sqlServerConnString,
            string sqlitePath, string password, string exportTables, bool isExportView, SqlConversionHandler handler,
          ...
          WaitCallback convert = delegate(object state)
                                        {
                                          ...
                                          {
                                            _isActive = true;
                                            ConvertSqlServerDatabaseToSQLiteFile(sqlServerConnString, sqlitePath, password, exportTables, isExportView, handler, selectionHandler, viewFailureHandler, createTriggers);
                                            _isActive = false;
        ...
        private static void ConvertSqlServerDatabaseToSQLiteFile(
            string sqlConnString, string sqlitePath, string password, string exportTables, bool isExportView, SqlConversionHandler handler,
            SqlTableSelectionHandler selectionHandler,
            FailedViewDefinitionHandler viewFailureHandler,
            bool createTriggers)
        {
            ...
            DatabaseSchema ds = ReadSqlServerSchema(sqlConnString, exportTables, isExportView, handler, selectionHandler);
        ...
        private static DatabaseSchema ReadSqlServerSchema(string connString, string exportTables, bool isExportView, SqlConversionHandler handler,
            SqlTableSelectionHandler selectionHandler)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                string exportTablesStr = "";
                //设置导出表集合条件
                if (exportTables.Length>0)
                {
                    exportTablesStr = " and TABLE_NAME IN (" + exportTables + ")"; 
                }

                // This command will read the names of all tables in the database
                SqlCommand cmd = new SqlCommand(@"select * from INFORMATION_SCHEMA.TABLES  where TABLE_TYPE = 'BASE TABLE'" + exportTablesStr, conn);
